...
PhaseColorTable.prototype = {
	...
	for( var i=0; i<fineness; i++ ) {
	    ...
	    var color = this.lookup( Math.cos(theta), Math.sin(theta) );
	    redTable[i] = Math.floor(color[0]);
	    greenTable[i] = Math.floor(color[1]);
	    blueTable[i] = Math.floor(color[2]);
	}

	this.lookup = function(real,imag) {
	    var index = Math.floor((Math.atan2(imag,real) + Math.PI) * fineness / 2 / Math.PI);
	    return [redTable[index], greenTable[index], blueTable[index]];
	};
    },
};

phaseColorTable = new PhaseColorTable(function(real,imag) {
    var theta = (Math.atan2(imag,real) + Math.PI) % (2 * Math.PI);
...
ModulusColorTable.prototype = {
	...
	this.lookup = function(real,imag) {
	    var scaled = 0.5 + Math.atan(Math.sqrt( real*real + imag*imag )) / (Math.PI);
...
modulusColorTable = new ModulusColorTable(function(real,imag) {
...
infinityColorTable = new ColorTable(function(real,imag) {
    var modulus = Math.sqrt( real*real + imag*imag );
...
function MultipliedColorTable(table1,table2) {
    this.lookup = function(real,imag) {
	var color1 = table1.lookup(real,imag);
	var color2 = table2.lookup(real,imag);
